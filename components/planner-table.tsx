                    ...
                    <div key={sector.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div className="bg-dark-primary text-white px-6 py-4 flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                {activeTab === "TABLE" && (
                                    <button onClick={() => handleAddProcess(sector.id)} className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-md text-sm font-medium transition-colors">
                                        <Plus className="h-4 w-4" /> Nova Etapa
                                    </button>
                                )}
                                <button onClick={() => handleDeleteSector(sector.id)} className="flex items-center gap-2 bg-red-500/20 hover:bg-red-500/40 text-red-300 px-3 py-1.5 rounded-md text-sm font-medium transition-colors">
                                                            ...
                                                            <table className="w-full text-sm text-left whitespace-nowrap">
                                                                ...
                                                                <tbody>
                                                                    {process.tasks.length === 0 ? (
                                                                        <tr><td colSpan={9} className="px-4 py-3 text-center text-gray-400 italic">Sem atividades.</td></tr>
                                                                    ) : (
                                                                        process.tasks.map((task: any) => (
                                                                            <tr key={task.id} className="border-b border-gray-50 hover:bg-gray-50/50 group">
                                                                                {/* Nome — clicável para abrir o drawer */}
